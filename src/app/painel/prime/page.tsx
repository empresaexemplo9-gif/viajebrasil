...
import { emTrial, planoVigente } from '@/lib/usuarios';
import { obterPlano, limiteCurriculos } from '@/lib/planos';
import { candidatos } from '@/lib/candidatos';
import { ranquear } from '@/lib/classificacao';
import { vagas } from '@/lib/dados';

export const metadata = { title: 'Painel Prime' };

export default function PainelPrimePage({
  searchParams,
}: {
  searchParams?: { ok?: string; vaga?: string };
}) {
  const usuario = usuarioAtual();
  if (!usuario) redirect('/entrar?proximo=/painel/prime');

  const chave = planoVigente(usuario);
      ...
      <div className="container-app py-16">
        <div className="cartao mx-auto max-w-lg text-center">
          ...
          <p className="mt-2 text-slate-600">
            Assine o Prime para a IA ler e ranquear todos os currículos das suas vagas, entregar
            os melhores talentos e medir sua visibilidade.
          </p>
  ...
  const ranking = vagaSel ? ranquear(candidatos, vagaSel) : [];
  const analisados = Math.min(ranking.length, plano.curriculosMes);
  const visiveis = ranking.slice(0, analisados);
  const scoreMedio = visiveis.length
    ? Math.round(visiveis.reduce((s, a) => s + a.score, 0) / visiveis.length)
    : 0;

  // Métricas de visibilidade (demonstração determinística por nível do plano).
  const exibicoes = analisados * (40 + plano.nivel * 35);
  ...
  return (
    <div className="container-app py-12">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {emTrial(usuario) && (
          <span className="selo bg-amber-100 text-amber-700">Em teste grátis</span>
        )}
      </div>

      {searchParams?.ok && (
        <p className="mt-4 rounded-lg bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-700">
          {searchParams.ok === 'trial'
            ? 'Teste grátis ativado. Aproveite todos os recursos Prime.'
            : 'Assinatura Prime ativada.'}
        </p>
      )}

      {/* Métricas */}
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Metrica rotulo="Currículos analisados" valor={String(analisados)} nota={`limite ${limiteCurriculos(plano)}/mês`} />
        <Metrica rotulo="Score médio" valor={String(scoreMedio)} nota="aderência à vaga" />
        <Metrica rotulo="Exibições × cliques" valor={`${exibicoes} · ${cliques}`} nota={`CTR ${ctr}%`} />
        <Metrica
          rotulo="ROI estimado"
      ...
      {/* Ranking de candidatos pela IA */}
      <h2 className="mt-8 text-xl font-black text-tinta">
        Talentos ranqueados pela IA {vagaSel ? `· ${vagaSel.titulo}` : ''}
      </h2>
      <p className="mt-1 text-sm text-slate-500">
        A IA leu os currículos e avaliou aderência, experiência, certificações e referências.
        Análise {plano.profundidade}.
      </p>

      <div className="mt-5 space-y-3">
        {visiveis.map((a, i) => (
          <div key={a.candidato.id} className="cartao flex flex-col gap-3 sm:flex-row sm:items-center">
            <div className="flex items-center gap-3">
              <span className="grid h-9 w-9 shrink-0 place-items-center rounded-lg bg-marca-50 text-sm font-black text-marca-700">
                {i + 1}º
              </span>
              <div>
                <h3 className="font-bold text-tinta">{a.candidato.nome}</h3>
                <p className="text-xs text-slate-500">
                  {a.candidato.area} · {a.candidato.regiao}
                </p>
              </div>
            </div>
    ...
    <div className="cartao">
      ...
      <div
        className={`mt-1 text-2xl font-black ${
          positivo === false ? 'text-rose-600' : 'text-tinta'
        }`}
      >
        {valor}
      </div>
        ...
        <div key={it.rotulo} title={`${it.rotulo}: ${it.v}/${it.max}`}>
          <div className="h-1.5 overflow-hidden rounded-full bg-slate-100">
            <div
              ...
              style={{ width: `${Math.round((it.v / it.max) * 100)}%` }}
            />
          </div>
        </div>
      ))}
